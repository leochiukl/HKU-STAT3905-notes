...
\item Binomial tree representation of the multi-period model (example):
\begin{center}
\begin{tikzpicture}
\node[] (s0) at (0,0) {\(S_0\)};
\node[] (su) at (4,1) {\(S_u=S_0u\)};
...
suggests the number of ``up'' and ``down'' (resp.) needed to get to that price
(the order of the letters is not important).  Sometimes to make the notation
more compact, we may express the subscript using power, e.g. \(S_{u^8d^2}\).
\item The rightmost stock prices in the binomial tree are known as
\defn{terminal stock prices}. For a \(n\)-period binomial option pricing model,
there are \(n+1\) different terminal stock prices.\footnote{This is because the
subscript of ``\(S\)'' can contain \(0,1,\dotsc,n\) ``\(u\)''s (with the rest
being ``\(d\)''s) \faIcon{arrow-right} \(n+1\) different subscripts.}
\end{remark}
...
\faIcon{apple-alt} and a short position in \faIcon{scroll} has also a constant
...
replicating portfolio} for \faIcon{scroll}.
...
\item Particularly, the expected return rate of the derivative \faIcon{scroll}
...
\item We can simplify the expression of the volatility to:
\[
\sqrt{\frac{1}{h}\vari{\ln S_h}}
\]
since \(\vari{\ln(S_h/S_0)}=\vari{\ln S_h-\ln S_0}\) and \(\ln S_0\) is
nonrandom.
...
see \cref{sect:bs-model,sect:bs-pricing-fmla}). %TODO: check
