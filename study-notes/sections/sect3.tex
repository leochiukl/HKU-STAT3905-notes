...
\begin{enumerate}
\item Recall the \emph{no-arbitrage principle} and \emph{perfect market}
mentioned in \labelcref{it:no-arbitrage-principle} and
\labelcref{it:perfect-mkt} respectively.
\item An important consequence of the no-arbitrage principle and the perfect
market assumption is the \emph{law of one price}.
...
\item Notations:
\begin{itemize}
\item \(r\): the annual risk-free interest rate compounded
continuously;
\item \(F_t\): forward/delivery price for a forward (with delivery date
...
current stock (spot) price is \(S_0\), and the stock \faIcon{apple-alt} will make dividend payment \faIcon{money-bill-wave}
...
\item Consider a forward on 1 \faIcon[regular]{gem}. Let \(C\) be \emph{present
