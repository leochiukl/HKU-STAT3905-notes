...
\text{\defn{\(LN(\mu,\sigma^2)\)}}\)) if the ``log'' of \(Y\), \(\ln Y\),
...
\end{align*}
where \defn{\(\Phi(\cdot)\)} is the standard normal cdf, and 
\[
...
(recall) and \(\text{\defn{\(\hat{d_1}\)}}=\hat{d_2}+\sigma\sqrt{T}.\)
\end{proposition}
\begin{intuition}
...
\end{align*}
where \defn{\(\phi(\cdot)\)} is the standard normal pdf.
\begin{center}
\begin{tikzpicture}
\begin{axis}[
domain=-3:3,
