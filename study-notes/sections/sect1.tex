...
\end{itemize}
\item The act of making a risk-free profit without any cash outflow (having a ``free
lunch'') is known as \defn{arbitrage}. Since taking offsetting positions would
not lead to any cash outflow, an \emph{arbitrage opportunity} arises if such act can
lead to a risk-free profit.
...
\begin{enumerate}
\item Having a \defn{long position} (\defn{short position}) in a financial
...
\item For brevity, we may simply use the word ``long'' to mean ``long position
in''. Example: ``long \faIcon{scroll}'' means ``long position in
\faIcon{scroll}''.
...
\emph{zero} \faIcon{scroll} is owned.

\item Example: if \faIcon{user} has a long position in \faIcon{scroll} (with 10
\faIcon{scroll} owned), then \faIcon{user} can short (i.e., ``sell'' in this
context) 10 \faIcon{scroll} to close out his long position.

\item The \defn{value} or \defn{payoff} of a position in financial instrument
\faIcon{scroll} at a certain time is the amount of cash \faIcon{dollar-sign}
received if all positions (including those ``extra'' positions in other
instruments created from closing out the position in \faIcon{scroll}) were
...
\item ``Value of one unit of \faIcon{scroll}'' means ``value of a long position
in \faIcon{scroll} (with one unit owned)''.
\item ``Value of one unit of \faIcon{scroll}'' also has the meaning of
``\emph{price} of one unit of \faIcon{scroll}'' (so sometimes ``price'' is used
instead of ``value''), since the amount of cash \faIcon{dollar-sign} received
from closing out a long position in \faIcon{scroll} (1 unit owned) (i.e.,
``selling'' 1 \faIcon{scroll}) is simply the ``price'' of 1 \faIcon{scroll}.
\item Unless stated
otherwise, the time unit is always years in this notes.
\end{remark}
\end{enumerate}
