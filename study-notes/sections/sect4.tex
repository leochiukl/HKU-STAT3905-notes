...
\faIcon{apple-alt} even \emph{before} maturity.
\end{itemize}
\end{enumerate}
...
\emph{European}. (This is referred to as its \defn{exercise style}.) Their
difference is on the buy/sell timing:
\begin{itemize}
\item The holder of an \defn{American} option can exercise at any time at or before the expiration date.
...
\end{itemize}
\end{enumerate}
\subsection{Payoffs of Call and Put Options}
\begin{enumerate}
...
in such a way that maximizes the holder's benefit \faIcon{arrow-right} every
...
\labelcref{it:amer-option-rational}.
\end{note}
...
the choice \faIcon{arrow-right} the ``choice'' vanishes afterward).
...
part} of \(x\), i.e., \(\max\{x,0\}\) (or \(x\vee 0\)).
...
\begin{enumerate}
\item \label{it:bull-strat}
For speculators that are \emph{bullish} on \faIcon{apple-alt}, they can:
\begin{itemize}
...
\item \label{it:bear-strat}
For speculators that are \emph{bearish} on \faIcon{apple-alt}, they can:
\begin{itemize}
